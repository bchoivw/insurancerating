...
#' @export
fit_gam_full2 <- function(data, xvar, yvars, model = "pure_premium",spline="cc"){

  nclaims = yvars[["nclaims"]]
  exposure = yvars[["exposure"]]
  loss = yvars[["loss"]]
  pure_premium = yvars[["pure_premium"]]
...
#' @export
autoplot_full2.fitgam <- function(gam_model, data, xvar, yvars, model = "pure_premium",
                                 round_x = NULL, x_min=NULL,x_max=NULL,y_min=NULL,y_max=NULL, remove_y_outliers = NULL,
                                 x_scale='identity',y_scale='identity', show_exposures = TRUE, show_observations = TRUE,x_stepsize = NULL,
                                 conf_int = TRUE, color_gam_line = "red", color_points = "black", rotate_labels = FALSE, ...){
